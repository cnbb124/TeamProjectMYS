...
public abstract class ProjectileData : ItemData
{
	...
	// 피격 이펙트는 탄종마다 의미가 달라 베이스에서 제거함:
	//  - 총알: BulletData.hitEffectType(일반) / shieldHitEffectType(실드) 2종
	//  - 미사일: MissileData.explodeEffectType(폭발) 1종
	[Tooltip("발사 시 VFX매니저에서 실행할 총구 머즐플래시 이펙트. 기본값은 일반 총알 머즐.")]
	...
	// 실드 피격음(shieldHitSoundType)도 여기 없음 — 탄종별로 안 나누고 Unit.GetPlaySoundTypeShield(HitInfo)에서 DAMAGE_TYPE 기준으로 일괄 처리.
